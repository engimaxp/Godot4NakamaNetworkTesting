...
var my_session_id: String:
	get = get_my_session_id
var match_id: String:
	get = get_match_id
var matchmaker_ticket: String:
	get = get_matchmaker_ticket
...
func start_matchmaking(_nakama_socket: NakamaSocket, data: Dictionary = {}) -> void:
	...
	var result = await nakama_socket.add_matchmaker_async(\
		data.get('query', '*'), data['min_count'], \
		data['max_count'], data.get('string_properties', {}), \
		...
		if match_mode == MatchMode.CREATE:
			...
			if players.size() < max_players:
				...
				nakama_socket.send_match_state_async(match_id, MatchOpCode.JOIN_SUCCESS, JSON.stringify({
					players = NakamaOnlineMatch.serialize_players(players),
...
func _on_nakama_match_state(data: NakamaRTAPI.MatchData):
	...
	if data.op_code == MatchOpCode.JOIN_SUCCESS && match_mode == MatchMode.JOIN:
		...
		var content_players = NakamaOnlineMatch.unserialize_players(content['players'])
